...
class CameraManager extends EventEmitter {
  ...
  async discoverAllCameras() {
    ...
    try {
      ...
      // Format detected cameras for the API response
      const formattedDetectedCameras = detectedCameras.map((camera, index) => ({
      ...
      // Preserve manually registered test cameras and merge with detected cameras
      const manualCameras = (this.availableCameras || []).filter(camera => 
        camera.detectionMethod === 'manual_test'
      );
      
      // Combine detected cameras with manual test cameras
      this.availableCameras = [...formattedDetectedCameras, ...manualCameras];
